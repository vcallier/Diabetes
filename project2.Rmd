...
When we do a cross validation by splitting the data into test and validation sets, the model accurately predicts diabetic status only about 57 percent of the time. The mean test error rate is 43.29 percent, and I plotted the distribution of test error rate. 
...
In summary, the training error rate is 22 percent, the test error rate is about 43 percent, and the cross-validation error rate is about 35 percent. 
...
The test error rate for the logistic GAM regression is 43.87 percent. This is not an improvement on the logistic linear regression above. The additional complexity of this GAM model did not improve the prediction ability, so this suggests that there may be better models to use. 
